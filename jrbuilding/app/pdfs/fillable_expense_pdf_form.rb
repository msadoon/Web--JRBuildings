class FillableExpensePdfForm
  ...
  def export(output_file_path=nil, tenantname, current_user)
    output_path = output_file_path || "tmp/#{tenantname}_expense_addendum_#{Time.now}.pdf" # make sure tmp/pdfs exists
  ...
  def template_path(current_user)
    ...
    file = open(User.expadd_url(current_user))
